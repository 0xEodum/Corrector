...
// CreateDictionaryEntry is the exported wrapper for adding a single
// dictionary entry programmatically while building the index.
// It mirrors the behavior of LoadDictionary's internal insertion path.
func (s *SymSpell) CreateDictionaryEntry(term string, count int) bool {
    return s.createDictionaryEntry(term, count)
}
