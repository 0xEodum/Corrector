// internal/lookup.go - исправленная версия
...
import (
	...
	"corrector/pkg/items"
	verbositypkg "corrector/pkg/verbosity"
)

// Вспомогательная функция max (для совместимости с более старыми версиями Go)
...
func (s *SymSpell) Lookup(
	...
	// ИСПРАВЛЕНИЕ: Если нашли точное совпадение с достаточной частотой, завершаем поиск
	...
	// ИСПРАВЛЕНИЕ: Используем улучшенную логику финализации
	s.finalizeWithImprovedFrequencyCheck(&cp, exactMatch.exactItem)
...
func (s *SymSpell) checkExactMatch(phrase string, verbosity verbositypkg.Verbosity, cp *candidateProcessor) ExactMatchResult {
	if count, found := s.Words[phrase]; found {
		...
		// ИСПРАВЛЕНИЕ: Изменяем логику для точных совпадений
		// Если verbosity не All и слово имеет минимальную частоту (больше 0), завершаем поиск
		if verbosity != verbositypkg.All {
			// Используем более низкий порог для точных совпадений
			minFrequencyForExact := max(1, s.FrequencyThreshold/100) // 1% от порога или минимум 1
			if count >= minFrequencyForExact {
				return ExactMatchResult{shouldStop: true, exactItem: &exactItem}
			}
		}

		// Для очень низкочастотных слов продолжаем поиск
		return ExactMatchResult{shouldStop: false, exactItem: &exactItem}
	}
	return ExactMatchResult{shouldStop: false, exactItem: nil}
}

// ИСПРАВЛЕНИЕ: Новая улучшенная функция для финализации с учетом частотности
func (s *SymSpell) finalizeWithImprovedFrequencyCheck(cp *candidateProcessor, exactMatch *items.SuggestItem) {
	...
	minSignificantDifference := max(s.FrequencyMultiplier, 5) // Минимальная значимая разница в частоте
	...
	for i := range cp.suggestions {
		...
		if suggestion.Distance <= 2 {
			// ИСПРАВЛЕНИЕ: Изменяем логику сравнения частотности
			// Альтернатива должна быть ЗНАЧИТЕЛЬНО частотнее точного совпадения
			frequencyRatio := float64(suggestion.Count) / float64(exactMatch.Count)

			// Альтернатива должна быть минимум в N раз частотнее И иметь существенную разность
			if frequencyRatio >= float64(s.FrequencyMultiplier) &&
				suggestion.Count >= exactMatch.Count+minSignificantDifference {
	...
	if bestAlternative != nil {
		...
		// fmt.Printf("Заменяем '%s' (%d) на '%s' (%d) из-за значительно большей частотности\n",
